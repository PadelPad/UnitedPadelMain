...
if (!(Test-Path $AppPath)) { throw "App path not found: $AppPath" }
...
if (Test-Path $android) {
  ...
  if (Test-Path $manifest) {
    ...
    if ($content -notmatch "app.unitedpadel.co.uk") {
      ...
      Warn "Manifest already contains deep link patterns; skipped"
    }
  }
...
if (Test-Path $ios) {
  ...
  Warn "Open the .xcworkspace, add the entitlements file in Signing & Capabilities, enable Push + Associated Domains."
